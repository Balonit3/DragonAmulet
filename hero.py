...
class Hero():
    ...
    def strike_gnom(self,enemy_gnom):
        ...
        if enemy_gnom.armor < 0:
            ...
            enemy_gnom.armor = 0
        print(enemy_gnom.name,'получил удар\nкласс брони упал',enemy_gnom.armor,'Уровень здоровья упал',enemy_gnom.health,'\n')
...
def start_history():
    ...
    sleep(2)
    print(bordered('Ричард двинулся в путь и пройдя через реки и поля он наконец то подходит к следующему припядствию. '))
    sleep(2)
    print(bordered('Это был тёмный и мрачный лес.Ричарду было страшно в него заходить,но он вспомнил про свою цель и не задумываясь вступил в него.'))
    sleep(2)
    print(bordered('И вот пройдя уже по леса он подумал, что дальше будет также легко,он даже удивился почему этот лес все так боялись.'))
    sleep(2)
    print(bordered('Но вдруг из гущи леса на него выбегают 3 гнома которые требуют с него 3 золотые монеты,либо они его убьют.'))
    sleep(2)
    ...
    sleep(2)
    print(bordered('это было зелье исцеление которое он мог выпить в любой следующей битве что бы восполнить своё здоровье '))
    sleep(2)
    ...
    sleep(2)
    print(bordered('И вот спустя 3 дня непрерывной дороги он видит ущелье в котором очень странно светится ярко-зелёным цветом река и скалы.'))
    sleep(2)
    print(bordered('Пройдя немного дальше он слышит громкий топот и сопение и повернув голову влево он видит его,того самого дракона про которого говорил кузнец.'))
    sleep(2)
    print(bordered('Весь покрыт плотной чешуёй тёмно-зелёного цвета дракон идёт в сторону Ричарда и Ричард понимает что битвы не избежать.'))
    sleep(2)
    print(bordered('И ричард делает первый рывок в его сторону и наносит своим мечом первый удар в зону живота,но из за плотной чешуи дракон не чувствует удара и атакует в ответ.'))
    sleep(2)
