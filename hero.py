...
def start_history():
    print(bordered('история Ричарда началась когда его отец не мало известный рыцарь погиб от лап дракона', color_code_green))
    sleep(2)
    print(bordered('Ричарду на тот момент было 5 лет и с тех пор он решил,что он должен отомстить.', color_code_green))
    sleep(2)
    print(bordered('И вот шли годы на протяжении всего этого времени он тренировался не покладая рук.', color_code_green))
    sleep(2)
    print(bordered('И вот с той трагедии прошло 12 лет и Ричард решил,что время мести настало.', color_code_green))
    sleep(2)
    print(
        bordered('Ричард двинулся в путь и пройдя через реки и поля он наконец то подходит к следующему припядствию. ', color_code_green))
    sleep(2)
    print(bordered(
        'Это был тёмный и мрачный лес.Ричарду было страшно в него заходить,но он вспомнил про свою цель и не задумываясь вступил в него.', color_code_green))
    sleep(2)
    print(bordered(
        'И вот пройдя уже по леса он подумал, что дальше будет также легко,он даже удивился почему этот лес все так боялись.', color_code_green))
    sleep(2)
    print(bordered(
        'Но вдруг из гущи леса на него выбегают 3 гнома которые требуют с него 3 золотые монеты,либо они его убьют.', color_code_green))
    sleep(2)
    ...
    sleep(2)
    print(bordered('Но по скольку этот лес не простой вместо желанной бутылочки пивчанского он получил суровый бой.', color_code_green))
    sleep(2)
    print(bordered('Джо не обладал сверх навыками но был готов дать отпор Ричарду за свой божественный напиток ', color_code_green))
    sleep(2)
    print(bordered('После этой схватки у Ричардо появился новый предмет как ПИВО,', color_code_green))
    sleep(2)
    print(bordered(
        'это было зелье исцеление которое он мог выпить в любой следующей битве что бы восполнить своё здоровье ', color_code_green))
    sleep(2)
    ...
    sleep(2)
    print(bordered('Кузнец ему рассказал что там обитает старый вид драконов который очень опасен из за своего яда.', color_code_green))
    sleep(2)
    print(bordered('Но для ричарда его слова были лишь мотивацией пойти туда.', color_code_green))
    sleep(2)
    print(bordered(
        'И вот спустя 3 дня непрерывной дороги он видит ущелье в котором очень странно светится ярко-зелёным цветом река и скалы.', color_code_green))
    sleep(2)
    print(bordered(
        'Пройдя немного дальше он слышит громкий топот и сопение и повернув голову влево он видит его,того самого дракона про которого говорил кузнец.', color_code_green))
    sleep(2)
    print(bordered(
        'Весь покрыт плотной чешуёй тёмно-зелёного цвета дракон идёт в сторону Ричарда и Ричард понимает что битвы не избежать.', color_code_green))
    sleep(2)
    print(bordered(
        'И ричард делает первый рывок в его сторону и наносит своим мечом первый удар в зону живота,но из за плотной чешуи дракон не чувствует удара и атакует в ответ.', color_code_green))
    sleep(2)
